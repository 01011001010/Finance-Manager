...
// Set-up
const { reloadLogTransactions } = getData(); // TODO tags, not transactions
...
const onUpload = async (event) => {
  ...
  try {
    ...
    const message = response.detail || "Tags uploaded successfully";
    successToast(message);

    // await reloadLogTransactions();  // TODO tags, not transactions
...
<template>
  <FileUpload
    mode="basic"
    name="file"
    url="/api/finance/bulk/upload/tags"
    accept=".csv"
    :maxFileSize="1000000"
    @upload="onUpload"
    @error="onUploadError"
    :multiple="false"
    :auto="true"
    chooseLabel="Import tags (.csv)"
    chooseIcon="pi pi-upload"
    :chooseButtonProps="{ fluid: true }"
  />
  <!-- TODO Maybe use same icon from drawer menu for tag creation-->
</template>
