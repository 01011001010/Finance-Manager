...
// Set-up
const { reloadLogTransactions } = getData(); // TODO also accounts and tags

// TODO give hint on hover about needed columns/files
...
const onUpload = async (event) => {
  ...
  try {
    ...
    const message = response.detail || "Data uploaded successfully";
...
<template>
  <FileUpload
    ...
    :multiple="false"
    :auto="true"
    chooseLabel="Import data (.zip)"
    chooseIcon="pi pi-upload"
    :chooseButtonProps="{ fluid: true }"
  />
  <!-- TODO Maybe use some additional-->
</template>
